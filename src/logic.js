// QVAC Breakup Text Generator — core logic.
// completion() writes the result from a single user input field.
// The gentleness meter is a deterministic score from keyword density
...
const FALLBACK = (v) => `I've been thinking about us and I don't think we're right for each other anymore, especially with ${v}. I care about you and I hope we can both move forward with kindness.`;
...
export async function generate(modelId, input) {
  const run = completion({
    ...
    history: [
      {
        role: "system",
        content: ((v) => `Write one short, kind, and clear breakup text (2-3 sentences) for this situation: ${v}. No cruelty, no insults, be direct but compassionate. Reply with ONLY the text, no preamble.`)(input),
      },
  ...
  return { result, gentleness: scoreGentleness(result) };
}

const KEYWORDS = ["care", "kind", "hope", "respect", "appreciate", "grateful", "understand"];

function scoreGentleness(text) {
  const lower = text.toLowerCase();
  let score = 45;
