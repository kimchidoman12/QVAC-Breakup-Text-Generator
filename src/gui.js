#!/usr/bin/env node
// QVAC Breakup Text Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29353;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/breakup") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Breakup Text Generator GUI ready at http://localhost:${PORT}`);
  });
